...
# `@observe` under `@tool` gives Langfuse a span that captures the query,
# limit, and returned items explicitly. Strands' tool instrumentation names
# the span but doesn't always surface the full payload — this makes the RAG
# result visible when you click into the trace.
@tool
